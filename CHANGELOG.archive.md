# Changelog Archive — copilot-instructions-template

Historical entries moved out of [CHANGELOG.md](CHANGELOG.md) to keep the active release log focused on the current `0.x` line.

This archive preserves older history from `v5.12.0` back to `v1.0.0`.
...
* **audit:** enhance copilot audit for missing PowerShell scripts and invalid customization paths ([da639b7](https://github.com/asafelobotomy/copilot-instructions-template/commit/da639b715aa1830375dfccbba9dd05449fe2ab5d))
...
* release version 4.2.0 with static audit tool and extended coverage for starter kits ([50ab6e5](https://github.com/asafelobotomy/copilot-instructions-template/commit/50ab6e5d7b2c580a6a930ee7dcdc3b46b0a7f1f8))
* **release:** enhance release workflow with concurrency and additional config options ([298ee50](https://github.com/asafelobotomy/copilot-instructions-template/commit/298ee50c8652e84ddf492f8bc5381e31f8dd02e7))
...
* **docs:** resolve markdownlint errors in agent and research docs ([c50704c](https://github.com/asafelobotomy/copilot-instructions-template/commit/c50704c51c45993a156a3698c2f54e7ab8cd0f92))
...
* **release:** correct conditional check for release job execution ([e6c07b3](https://github.com/asafelobotomy/copilot-instructions-template/commit/e6c07b32f0134f146a144e8d0ffd0b37aa50f712))
...
- `tests/scripts/test-security-edge-cases.sh` — 29-assertion security and contract edge-case suite for `guard-destructive.sh` and `verify-version-references.sh` (formerly `sync-version.sh`). Covers six gap categories identified by online research (OWASP Command Injection cheat sheet, BATS testing best practices): exit-code contract (hook must always exit 0), JSON output validity (every response parseable), `tool_input.input` field alias support, OWASP-sourced chained/embedded command detection (`;`, `&&`, `||`, subshell), SQL keyword case-insensitivity, and `verify-version-references.sh` idempotency.
...
- The older legacy suites (`test-hooks.sh`, `test-guard-destructive.sh`, `test-verify-version-references.sh` (then `test-sync-version.sh`), and `test-security-edge-cases.sh`) now also use the shared helper layer, and `tests/run-all.sh` is grouped into labeled phases so local runs are easier to scan.
...
- Added `scripts/release/verify-version-references.sh` (then `sync-version.sh`) to propagate `VERSION.md` into derived references (`.github/copilot-instructions.md` stamp, `README.md` version badge, `.release-please-manifest.json`). CI now enforces these derived files are synced.
...
- `docs/UPDATE-GUIDE.md` — stale section-range wording in the v1.x→v2.0.0 migration note changed to "sections 1 through 12"; resolves cross-reference consistency CI failure.
...
- `.github/workflows/ci.yml` — the then-current 12-section check updated to the then-current 13-section check; `docs/MCP-GUIDE.md`, `docs/RELEASE-AUTOMATION-GUIDE.md`, and `template/vscode/mcp.json` added to required files check.
...
- `CONTRIBUTING.md` — CI checklist reference updated from the then-current 12-section range to the then-current 13-section range.
...
- `docs/INSTRUCTIONS-GUIDE.md` — updated from 12-section wording to 13-section wording; §13 MCP section guide added.
...
- `CONTRIBUTING.md` — corrected a stale CI checklist reference from the then-current 11-section range to the then-current 12-section range (§12 was added in v1.1.0 but CONTRIBUTING.md was not updated).
...
- `README.md` — version badge `1.1.0`; "Twelve-section" heading; "📚 Agent Skills library" feature block; skills scaffolding entry in "What gets scaffolded" table; `SKILLS-GUIDE.md` in docs table; layout tree expanded with `skills/` directories and `SKILLS-GUIDE.md`; then-current section references updated throughout.
- `.github/workflows/ci.yml` — the then-current 11-section check updated to the then-current 12-section check; `docs/SKILLS-GUIDE.md` added to required files.
- `docs/INSTRUCTIONS-GUIDE.md` — updated from 11-section wording to 12-section wording; added full §12 writeup with Scan/Search/Create stages and customisation guidance.
