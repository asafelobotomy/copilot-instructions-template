...
- **Read**: Call `asafelobotomy_read_diaries(agent_name)` for a specific agent's entries, or `asafelobotomy_read_diaries()` for all agents. `asafelobotomy_spatial_status` also surfaces a recent summary.
...
- **Hook**: SubagentStart injects a hint to call `asafelobotomy_spatial_status`.
