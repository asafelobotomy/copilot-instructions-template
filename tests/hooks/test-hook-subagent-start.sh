...
output=$(printf '%s' '{"agentName":"Review-Agent_v2"}' | bash "$SCRIPT" 2>/dev/null)
status=$?
...
payload='{"agentName":"Review \"Quoted\""}'
...
TMP=$(mktemp -d); CLEANUP_DIRS+=("$TMP")
output=$(cd "$TMP" && printf '%s' '{"agentName":"Code"}' | bash "$SCRIPT" 2>/dev/null)
status=$?
