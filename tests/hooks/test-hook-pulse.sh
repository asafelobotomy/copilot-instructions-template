...
echo "7. user accepting retrospective keyword blocks at stop"
...
echo "10. discussion-only prompt does not arm heartbeat or retrospective"
...
echo "11. session_start captures git baseline"
...
echo "12. soft_post_tool increments edit count only for file-writing tools"
...
echo "13. soft_post_tool opens and accumulates work windows"
...
echo "14. pure planning session (0 file changes) never triggers retrospective"
...
echo "15. pre-existing dirty files do not inflate the delta count"
...
echo "16. corrupt state file is recovered safely"
...
echo "17. transition digest stays quiet while the session is only orienting"
...
echo "18. transition digest appears when work crosses into consolidating"
...
echo "19. session_start initializes transition state"
