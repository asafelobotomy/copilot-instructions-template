...
> Role: AI developer for this repository. Template version: 0.8.0 <!-- x-release-please-version --> | Updated: 2026-04-28
>
...
- **MCP Protocol**: Config in `.vscode/mcp.json`. Tiers — Always-on: `filesystem`, `git`. Project tools (enabled in dev): `heartbeat` (session reflect + diary), `fetch`, `context7`, `duckduckgo`. Optional (enabled in dev): `sequential-thinking`. Credentials-required (disabled by default): `github` (GitHub API — issues, PRs, Actions, Dependabot; enable when VS Code OAuth access is needed).
