...
- **MCP Protocol**: Config in `.vscode/mcp.json`. Tiers — Always-on: `filesystem`, `git`. Project tools (enabled in dev): `heartbeat` (session reflect + diary), `fetch`, `context7`. Optional (enabled in dev): `sequential-thinking`, `playwright`. Credentials-required (disabled by default): `github` (GitHub API — issues, PRs, Actions, Dependabot; enable when VS Code OAuth access is needed).
