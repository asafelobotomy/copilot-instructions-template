...
> Role: AI developer for this repository. Template version: 0.10.0 <!-- x-release-please-version --> | Updated: 2026-04-28
>
...
0. **Route** — call `mcp_heartbeat_suggest_delegation(task="<one-sentence description>")`. If it returns `match: true`, delegate to the named agent immediately — do not absorb the specialist workflow inline. For sub-agent calls, also pass `calling_agent` to avoid self-routing. Skip this step only for trivial single-line lookups (e.g. "what does this regex match?").
...
- Parity — skills: `template/skills/` must stay byte-identical with `.github/skills/` (direct CI check via `validate-consumer-surface-parity.sh`). Parity — instructions/prompts: `.github/instructions/` and `.github/prompts/` must stay in sync with their `template/` counterparts (enforced via `test-customization-contracts-surfaces.sh`).
...
- Main/default agent delegation: call `mcp_heartbeat_suggest_delegation` before any non-trivial task (step 0 of the Structured Thinking Discipline). If it returns a match, delegate instead of absorbing the specialist workflow inline.
...
- **MCP Protocol**: Config in `.vscode/mcp.json`. Tiers — Always-on: `filesystem`, `git` (local Python server at `hooks/scripts/mcp-git-server.py` — owned copy with all upstream fixes applied; repo locked to `${workspaceFolder}` via `GIT_MCP_REPOSITORY` env var). Project tools (enabled in dev): `heartbeat` (session reflect + diary + `run_tests` — call `mcp_heartbeat_run_tests` to run targeted or full test suites; prefer over `runCommands` + bash for all test invocations), `fetch` (owned SSRF-protected server at `hooks/scripts/mcp-fetch-server.py` — blocks loopback, private, link-local ranges), `docs` (DevDocs documentation lookup — call `mcp_docs_resolve_library_id` to find a slug, then `mcp_docs_query_docs` to fetch entries; covers 794+ doc sets), `duckduckgo`, `sequential-thinking` (call `mcp_sequential-th_sequentialthinking` for complex planning tasks, architectural decisions, or any task requiring explicit thought branching, backtracking, or a shareable reasoning trace; use the built-in Structured Thinking Discipline for standard single-turn work). Credentials-required (disabled by default): `github` (GitHub API — read-only, toolsets: issues, pull_requests, actions, dependabot; enable when VS Code OAuth access is needed. For write operations, change URL from `.../readonly` to `https://api.githubcopilot.com/mcp/` before enabling).
