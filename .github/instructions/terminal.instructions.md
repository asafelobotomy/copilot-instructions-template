...
- Do not pass shell names, terminal labels, or subagent results to terminal-session tools. Only async UUIDs and foreground numeric `terminalId` values are valid selectors.
...
- If you only need command output rather than a persistent background session, prefer a synchronous `run_in_terminal` over polling a background terminal.
...
- If `get_terminal_output` returns a "command not found" error, the call used an invalid terminal ID (an integer panel ID or a subagent result instead of an async UUID). Discard the result, do not retry with the same ID, and re-run the command using synchronous `run_in_terminal`.
- The VS Code sandbox injects a restricted PATH. Tools present in the user's interactive shell (e.g. `python`, `actionlint`, `shellcheck`) may be absent. Probe with `command -v <tool>` before invoking, or use `run_in_terminal` with `requestUnsandboxedExecution` when sandbox PATH is blocking progress.
