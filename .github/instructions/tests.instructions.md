...
- Name test files to mirror the source file they cover (e.g. `guard-destructive.sh` → `test-guard-destructive.sh`).
...
- Every test must emit output on both pass and fail — silent tests are forbidden. Call `finish_tests` at the end of every suite.
