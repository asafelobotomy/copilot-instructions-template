...
The heartbeat also includes a **Retrospective** — 8 structured self-reflection questions grounded in observable events that Copilot answers after completing a task. These questions help the agent refine its reasoning heuristics (SOUL.md), build its understanding of the user (USER.md), and consolidate lessons into long-term memory (MEMORY.md). Two questions (issue reports and agent questions) surface directly to the user; the rest are written silently to workspace files.
