...
6. For `npx`-based stdio servers on Linux/macOS, add `sandboxEnabled: true` with `sandbox.filesystem.denyRead` rules for credential directories (`~/.ssh`, `~/.gnupg`, `~/.aws`) as a defence-in-depth measure against prompt injection. Optionally add `sandbox.network.allowedDomains` to restrict outbound network access. Do not sandbox `uvx`-based servers (`git`, `fetch`, `heartbeat`): the VS Code sandbox proxy intercepts PyPI network access during the `uvx` launcher phase and triggers repeated domain-approval prompts that cannot be reliably suppressed via per-server `allowedDomains`.
