...
#
# Usage:
#   bash scripts/sync/sync-template-parity.sh --write   # copy template/ → .github/ for drifted files
#   bash scripts/sync/sync-template-parity.sh --check   # fail if any mirror is out of sync
set -euo pipefail
...
require_python_check_write "sync/sync-template-parity.sh" "$MODE"
...
if mode == "--check":
    ...
    print("Run: bash scripts/sync/sync-template-parity.sh --write")
    sys.exit(1)
